...
for pf in ${parfile_array[@]}; do 
    python $PROJECT/docker/entrypoint.py getparfile --lapsolve-par "carpf_laplace_$pf" --debug --code-dir $PROJECT --base-dir "$laendo_dir/"
done
...
python $PROJECT/docker/entrypoint.py uac --uac-stage 1 --atrium la --fourch --layer endo --msh "$mname" --landmarks Landmarks.txt --regions Regions.txt --scale 1000 --debug --code-dir $PROJECT --base-dir "$laendo_dir/"
...
echo "=====New UAC====="
python $PROJECT/docker/entrypoint.py uac --uac-stage 2a --atrium la --fourch --layer endo --msh "$mname" --landmarks Landmarks.txt --regions Regions.txt --scale 1000 --debug --code-dir $PROJECT --base-dir "$laendo_dir/"
...
echo "=====New UAC part 2====="
python $PROJECT/docker/entrypoint.py uac --uac-stage 2b --atrium la --fourch --layer endo --msh "$mname" --landmarks Landmarks.txt --regions Regions.txt --scale 1000 --debug --code-dir $PROJECT --base-dir "$laendo_dir/"
...
echo "Copying file: carpf_laplace_* files"
for pf in ${parfile_array[@]}; do 
    python $PROJECT/docker/entrypoint.py getparfile --lapsolve-par "carpf_laplace_$pf" --debug --code-dir $PROJECT --base-dir "$laepi_dir/"
done
...
echo "=====Old UAC approximation====="
python $PROJECT/docker/entrypoint.py uac --uac-stage 1 --atrium la --fourch --layer epi --msh "$mname" --landmarks Landmarks.txt --regions Regions.txt --scale 1000 --debug --code-dir $PROJECT --base-dir "$laepi_dir/"
...
echo "=====New UAC====="
python $PROJECT/docker/entrypoint.py uac --uac-stage 2a --atrium la --fourch --layer epi --msh "$mname" --landmarks Landmarks.txt --regions Regions.txt --scale 1000 --debug --code-dir $PROJECT --base-dir "$laepi_dir/"
...
echo "=====New UAC part 2====="
python $PROJECT/docker/entrypoint.py uac --uac-stage 2b --atrium la --fourch --layer epi --msh "$mname" --landmarks Landmarks.txt --regions Regions.txt --scale 1000 --debug --code-dir $PROJECT --base-dir "$laepi_dir/"

echo "=====Add fibres2 ====="
python $PROJECT/docker/entrypoint.py fibremap --atrium la --layer epi --fibre l --msh "$mname" --output Fibre_Labarthe --debug --code-dir $PROJECT --base-dir "$laepi_dir/"

echo "=====Add LAT field====="
python $PROJECT/docker/entrypoint.py sims --lat-type normal --lat-file LAT_Spiral4_B.dat --msh Fibre_L --debug --code-dir $PROJECT --base-dir "$laepi_dir/"
...
echo "Copying file: carpf_laplace_* files"
for pf in ${parfile_array[@]}; do 
    python $PROJECT/docker/entrypoint.py getparfile --lapsolve-par "carpf_laplace_$pf" --debug --code-dir $PROJECT --base-dir "$raepi_dir/"
done
...
python $PROJECT/docker/entrypoint.py labels --labels-lndmrks --labels-stage 1 --labels-thresh 0.6 --msh RA_only --landmarks Landmarks.txt --regions Regions.txt --scale 1000 --debug --code-dir $PROJECT --base-dir "$raepi_dir/"
...
python $PROJECT/docker/entrypoint.py labels --labels-lndmrks --labels-stage 2 --labels-thresh 0.6 --msh RA_only --landmarks Landmarks.txt --regions Regions.txt --scale 1000 --debug --code-dir $PROJECT --base-dir "$raepi_dir/"

echo "=====Old UAC approximation====="
python $PROJECT/docker/entrypoint.py uac --uac-stage 1 --atrium ra --fourch --layer epi --msh $mname"_RAA" --landmarks Landmarks.txt --regions Regions.txt --scale 1000 --debug --code-dir $PROJECT --base-dir "$raepi_dir/"
...
echo "=====New UAC====="
python $PROJECT/docker/entrypoint.py uac --uac-stage 2a --atrium ra --fourch --layer epi --msh $mname"_RAA" --landmarks Landmarks.txt --regions Regions.txt --scale 1000 --debug --code-dir $PROJECT --base-dir "$raepi_dir/"
...
echo "=====New UAC part 2====="
python $PROJECT/docker/entrypoint.py uac --uac-stage 2b --atrium ra --fourch --layer epi --msh $mname"_RAA" --landmarks Landmarks.txt --regions Regions.txt --scale 1000 --debug --code-dir $PROJECT --base-dir "$raepi_dir/"
...
echo "Copying file: carpf_laplace_* files"
for pf in ${parfile_array[@]}; do 
    python $PROJECT/docker/entrypoint.py getparfile --lapsolve-par "carpf_laplace_$pf" --debug --code-dir $PROJECT --base-dir "$raendo_dir/"
done

meshtool convert -imsh=$raendo_dir/RA_endo.vtk -omsh=$raendo_dir/RA_only -ofmt=carp_txt -scale=1000

# # NEED TO SPLIT AS BEFORE & AFTER CARP
python $PROJECT/docker/entrypoint.py labels --labels-lndmrks --labels-stage 1 --labels-thresh 0.6 --msh RA_only --landmarks Landmarks.txt --regions Regions.txt --scale 1000 --debug --code-dir $PROJECT --base-dir "$raendo_dir/"
...
python $PROJECT/docker/entrypoint.py labels --labels-lndmrks --labels-stage 2 --labels-thresh 0.6 --msh RA_only --landmarks Landmarks.txt --regions Regions.txt --scale 1000 --debug --code-dir $PROJECT --base-dir "$raendo_dir/"

echo "=====Old UAC approximation====="
python $PROJECT/docker/entrypoint.py uac --uac-stage 1 --atrium ra --fourch --layer endo --msh $mname"_RAA" --landmarks Landmarks.txt --regions Regions.txt --scale 1000 --debug --code-dir $PROJECT --base-dir "$raendo_dir/"
...
echo "=====New UAC====="
python $PROJECT/docker/entrypoint.py uac --uac-stage 2a --atrium ra --fourch --layer endo --msh $mname"_RAA" --landmarks Landmarks.txt --regions Regions.txt --scale 1000 --debug --code-dir $PROJECT --base-dir "$raendo_dir/"
...
echo "=====New UAC part 2====="
python $PROJECT/docker/entrypoint.py uac --uac-stage 2b --atrium ra --fourch --layer endo --msh $mname"_RAA" --landmarks Landmarks.txt --regions Regions.txt --scale 1000 --debug --code-dir $PROJECT --base-dir "$raendo_dir/"

#Fibres next

echo "=====Add fibres2 (EPI) ====="
python $PROJECT/docker/entrypoint.py fibremap --atrium ra --layer epi --fibre l --msh $mname"_RAA" --output Fibre_Labarthe --debug --code-dir $PROJECT --base-dir "$raepi_dir/"

echo "=====Add LAT field ====="
python $PROJECT/docker/entrypoint.py sims --lat-type normal --lat-file LAT_Spiral4_B.dat --msh Fibre_Labarthe --debug --code-dir $PROJECT --base-dir "$raepi_dir/" 

python $PROJECT/docker/entrypoint.py scalarmap --atrium ra --layer epi --fibre l --msh RA_only_RAA --scalar-file Extra_SAN.dat --output MappedScalar_SAN --debug --code-dir $PROJECT --base-dir "$raendo_dir/"
python $PROJECT/docker/entrypoint.py scalarmap --atrium ra --layer epi --fibre l --msh RA_only_RAA --scalar-file Extra_CT.dat --output MappedScalar_CT --debug --code-dir $PROJECT --base-dir "$raendo_dir/"
python $PROJECT/docker/entrypoint.py scalarmap --atrium ra --layer epi --fibre l --msh RA_only_RAA --scalar-file Extra_PM.dat --output MappedScalar_PM --debug --code-dir $PROJECT --base-dir "$raendo_dir/"


echo "======Fibre mapping bilayer for LA======"
python $PROJECT/docker/entrypoint.py fibremap --atrium la --layer bilayer --fibre l --fourch --msh LA_only --msh-endo Labelled --msh-epi Labelled --output Fibre_Labarthe_Bilayer --debug --code-dir $PROJECT --base-dir "$laepi_dir/"

echo "Fibre mapping bilayer for RA"
python $PROJECT/docker/entrypoint.py fibremap --atrium ra --layer bilayer --fibre l --fourch --msh RA_only_RAA --msh-endo Labelled --msh-epi Labelled --output Fibre_Labarthe_Bilayer --debug --code-dir $PROJECT --base-dir "$raepi_dir/"
